...
function doPost(e) {
  try {
    ...
    if (action === "auto") {
      ...
      if (!us) return out({ ok: false, error: "sessao invalida" });
      return out({ ok: true, user: publicUser(us) });
    }
    if (action === "list") {
      var us2 = getSess(body.sess);
      if (!us2) return out({ ok: false, error: "sessao invalida ou expirada" });
      touchSess(body.sess);
      return out(readAll(us2));
    }
    if (action === "write") {
      var us3 = getSess(body.sess);
      if (!us3) return out({ ok: false, error: "sessao invalida ou expirada" });
      touchSess(body.sess);
      return out(writeRows(body.rows || [], us3));
    }
...
function publicUser(us) {
  return { usuario: us.u, nome: us.nome, perfil: us.perfil, escritorio: us.escritorio };
}
...
function readAll(us) {
  ...
  for (var i = HEADER_ROW + 1; i < data.length; i++) {
    ...
    todas.push(cells);
    if (ownerN === "" || ownerN === u || ownerN === un) minhas.push(cells);
  }
...
function writeRows(rowsIn, us) {
  ...
  for (var r = 0; r < rowsIn.length; r++) {
    ...
    if (!row) {
      ...
      if (ownerNow !== "" && ownerNow !== u && ownerNow !== un) {
